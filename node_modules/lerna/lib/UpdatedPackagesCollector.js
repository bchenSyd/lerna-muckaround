...
var UpdatedPackagesCollector = function () {
  ...
  _createClass(UpdatedPackagesCollector, [{
    ...
    value: function collectUpdatedPackages() {
      ...
      if (!since || forced.has("*")) {  // force-publish=*
        this.packages.forEach(registerUpdated);
      } else {
        this.packages.filter(function (pkg) {
          if (forced.has(pkg.name)) {  //force-publish=gel-button
            return true;
          }
          

          debugger; //real diff
          return _this.hasDiffSinceThatIsntIgnored(pkg, since);  // git diff --name-only $(git describe --tag --abbrev=0)  -- packages/xxx 
